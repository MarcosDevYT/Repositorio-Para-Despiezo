...
export type MatriculaResponse =
  | MatriculaResponseSolvedia
  | MatriculaResponseOscaro;
...
function normalizeVehicleData(response: MatriculaResponse) {
  ...
  if (isOscaroFormat(response)) {
    ...
    return {
      ...
      transmission: null,
      fuelType:
        data.version.includes("TDI") || data.version.includes("Diesel")
          ? "Diesel"
          : data.version.includes("TSI") || data.version.includes("Gasolina")
            ? "Gasolina"
            : null,
